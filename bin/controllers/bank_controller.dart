...
class BankController {
  ...
  bool makeTransfer(
    ...
    if (!verifyId(idReceiver)) {
      throw ReceiverIdInvalidExeption(idReceiver: idReceiver);
    }
    ...
    if (!accountReceiver.isAuthenticated) {
      throw ReceiverNotAuthenticatedExeption(idReceiver: idReceiver);
    }

    // Verificar se o remetente possui saldo suficiente
    if (accountSender.balance < amount) {
      throw SenderBalanceLowerThanAmountExeption(idSender: idSender, senderBalance: accountSender.balance, amount: amount);
    }
